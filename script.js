// ============================================================
// 🎯 AI BRAND STUDIO - COMPLETE SCRIPT
// ============================================================
...
function cleanColor(color) {
    ...
    return map[(color || '').toLowerCase().trim()] || '#1A1A1A';
}
...
function generateCanvasLogo(brandName, style, colorHex) {
    ...
    if (styleLower.includes('premium') || styleLower.includes('gold') || styleLower.includes('luxury')) {
        const gold = ctx.createLinearGradient(lx - 100, ly - 100, lx + 100, ly + 100);
        gold.addColorStop(0, '#FFD700');
        gold.addColorStop(0.3, '#FFF8DC');
        gold.addColorStop(0.5, '#FFD700');
        gold.addColorStop(0.7, '#DAA520');
        gold.addColorStop(1, '#B8860B');
        ctx.shadowColor = '#FFD700';
        ctx.shadowBlur = 35;
        ctx.fillStyle = gold;
        ctx.font = `bold ${letterSize}px 'Georgia', serif`;
        ctx.fillText(firstLetter, lx, ly);
        ctx.shadowBlur = 0;
        ctx.strokeStyle = '#B8860B';
        ctx.lineWidth = 2;
        ctx.strokeText(firstLetter, lx, ly);
        ...
        ctx.font = `bold ${letterSize}px 'Courier New', monospace`;
        ctx.fillText(firstLetter, lx - 4, ly - 4);
        ...
        ctx.stroke();
        const grad = ctx.createLinearGradient(lx - 100, ly - 100, lx + 100, ly + 100);
        ...
        for (let i = 0; i < 8; i++) {
            ...
            const x = lx + Math.cos(angle) * 160;
            const y = ly + Math.sin(angle) * 160;
            ctx.shadowBlur = 0;
            ctx.beginPath();
        ...
        for (let i = 0; i < 8; i++) {
            ...
            ctx.fillText(firstLetter, lx + offX, ly + offY);
        }
        ctx.globalAlpha = 1;
        ctx.shadowColor = 'rgba(0,0,0,0.3)';
        ctx.shadowBlur = 20;
        ...
        const cos = ctx.createRadialGradient(lx, ly, 20, lx, ly, 150);
        cos.addColorStop(0, '#ffffff');
        cos.addColorStop(0.3, '#FFD700');
        cos.addColorStop(0.6, '#7B2FBE');
        cos.addColorStop(1, '#1a0533');
        ctx.fillStyle = cos;
        ...
        const def = ctx.createLinearGradient(lx - 100, ly - 100, lx + 100, ly + 100);
        def.addColorStop(0, cleanColorHex);
        def.addColorStop(0.5, '#ffffff');
        def.addColorStop(1, cleanColorHex);
        ctx.shadowColor = cleanColorHex;
        ctx.shadowBlur = 25;
        ctx.fillStyle = def;
    ...
    ctx.fillText(brandName.toUpperCase(), lx, nameY + 20);

    // ─── VIGNETTE ───
    const vig = ctx.createRadialGradient(w / 2, h / 2, 200, w / 2, h / 2, 350);
...
function generateLogoUrl(brandName, style) {
    ...
    const rStyle = styleWords[Math.floor(Math.random() * styleWords.length)];
    const rExtra = extras[Math.floor(Math.random() * extras.length)];
    const rModel = models[Math.floor(Math.random() * models.length)];
    const seed = Math.floor(Math.random() * 100000);
    const prompt = `${brandName} ${style} ${rStyle} logo, ${rExtra}`;
    return `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?model=${rModel}&seed=${seed}&t=${Date.now()}`;
}
...
generateBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        history = history.filter(item => item.brandName !== brandNameInput.value);
        history.unshift({ brandName: brandNameInput.value, tagline: ai.tagline });
            ...
            <div style="display:flex;flex-direction:column;align-items:center;width:100%;">
                <div style="position:relative;width:280px;height:280px;border-radius:16px;overflow:hidden;background:rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;">
                    ...
                    <img 
                        ...
                        onload="this.style.opacity='1'; hideSpinner();"
                        onerror="this.style.display='none'; hideSpinner(); document.getElementById('canvasFallbackLogo').style.display='block';"
                    >
...
console.log('🎨 Canvas ready with', LOGO_STYLES.length, 'styles');
console.log('🔄 Reimagine generates DIFFERENT logos!');